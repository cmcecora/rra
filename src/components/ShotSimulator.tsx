...
function generateShotDescription(
	...
	const shotTypeLabel =
		SHOT_TYPES.find((t) => t.value === shotType)?.label || shotType;
	const techLabel =
	...
	const positionLabel =
		...
		courtPosition;
	const contextLabel =
		MATCH_CONTEXTS.find((c) => c.value === matchContext)?.label || matchContext;
	const opponentLevel =
		RATING_LEVELS.find((r) => r.value === opponentRating)?.label ||
		`${opponentRating} rating`;

	const rallyText =
		consecutiveShots > 1 ? ` during ${consecutiveShots}-shot rally` : '';
...
export default function ShotSimulator() {
	...
	const [technicalExecution, setTechnicalExecution] =
		useState<TechnicalExecution>('good-shot');
		...
		setTimeout(() => {
			...
			for (let i = 0; i < 4; i++) {
				const randomShotType =
					SHOT_TYPES[Math.floor(Math.random() * SHOT_TYPES.length)].value;
				const randomTechnicalExecution =
					TECHNICAL_EXECUTIONS[
						Math.floor(Math.random() * TECHNICAL_EXECUTIONS.length)
					].value;
				let randomOutcome =
					OUTCOMES[Math.floor(Math.random() * OUTCOMES.length)].value;
				const randomCourtPosition =
					COURT_POSITIONS[Math.floor(Math.random() * COURT_POSITIONS.length)]
						.value;
				const randomMatchContext =
					MATCH_CONTEXTS[Math.floor(Math.random() * MATCH_CONTEXTS.length)]
						.value;
				const randomConsecutiveShots =
					randomShotType === 'serve' ? 1 : Math.floor(Math.random() * 9) + 1;
				const randomOpponentRating =
					RATING_LEVELS[Math.floor(Math.random() * RATING_LEVELS.length)].value;

				if (
					randomTechnicalExecution === 'minor-error' ||
					randomTechnicalExecution === 'major-error'
				) {
				...
				const shotTypeLabel =
					SHOT_TYPES.find((t) => t.value === randomShotType)?.label || '';
				const techLabel =
					TECHNICAL_EXECUTIONS.find((t) => t.value === randomTechnicalExecution)
						?.label || '';
				const outcomeLabel =
					OUTCOMES.find((o) => o.value === randomOutcome)?.label || '';
				...
				<div className="grid grid-cols-12 gap-6 h-[calc(100vh-120px)]">
							...
							{QUICK_SCENARIOS.map((scenario, index) => (
								...
								>
									<div className="flex items-start justify-between mb-1.5">
										<div className="text-xs font-bold leading-tight pr-2">
											{scenario.name}
										</div>
									...
									<div className="text-[10px] text-blue-50 space-y-0.5">
										<div>
											{
												SHOT_TYPES.find(
													(t) => t.value === scenario.config.shotType
												)?.label
											}{' '}
											•{' '}
											{
												TECHNICAL_EXECUTIONS.find(
													(t) => t.value === scenario.config.technicalExecution
												)?.label
											}
										</div>
										<div>
											{
												OUTCOMES.find(
													(o) => o.value === scenario.config.outcome
												)?.label
											}{' '}
											•{' '}
											{
												MATCH_CONTEXTS.find(
													(c) => c.value === scenario.config.matchContext
												)?.label
											}
										</div>
									</div>
												...
												<div className="text-[10px] text-emerald-50 space-y-0.5">
													<div>
														...
														•{' '}
														{
															TECHNICAL_EXECUTIONS.find(
																(t) =>
																	t.value === scenario.config.technicalExecution
															)?.label
														}
													</div>
													<div>
					...
					<div className="col-span-5 bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col">
						<div className="bg-gradient-to-br from-slate-50 to-slate-100 border-b border-gray-200 p-4">
							...
							<p className="text-xs text-gray-500 mt-1">
								Configure parameters and calculate impact
							</p>
						</div>
							...
							<div className="grid grid-cols-2 gap-4">
								...
								<div>
									...
									<select
										...
										onChange={(e) =>
											setTechnicalExecution(
												e.target.value as TechnicalExecution
											)
										}
										className="w-full border-2 border-gray-300 rounded-lg px-3 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white hover:border-gray-400 transition-colors"
									>
								...
								<div>
									...
									<select
										value={courtPosition}
										onChange={(e) =>
											setCourtPosition(e.target.value as CourtPosition)
										}
										className="w-full border-2 border-gray-300 rounded-lg px-3 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white hover:border-gray-400 transition-colors"
									>
								...
								<div>
									...
									<select
										value={matchContext}
										onChange={(e) =>
											setMatchContext(e.target.value as MatchContext)
										}
										className="w-full border-2 border-gray-300 rounded-lg px-3 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white hover:border-gray-400 transition-colors"
									>
								...
								<div>
									...
									<input
										...
										onChange={(e) =>
											setConsecutiveShots(parseInt(e.target.value) || 1)
										}
										className="w-full border-2 border-gray-300 rounded-lg px-3 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white hover:border-gray-400 transition-colors"
									/>
								</div>
								...
								<div>
									...
									<select
										...
										onChange={(e) =>
											setOpponentRating(parseInt(e.target.value))
										}
										className="w-full border-2 border-gray-300 rounded-lg px-3 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white hover:border-gray-400 transition-colors"
									>
								...
								<div className="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl p-6 text-center">
									<div
										className={`text-6xl font-black mb-1 ${
											calculatedChange >= 0 ? 'text-green-600' : 'text-red-600'
										}`}
									>
										{calculatedChange >= 0 ? '+' : ''}
								...
								<div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-5 border border-slate-200">
									...
									<div className="space-y-2 text-xs">
										...
										<div className="flex justify-between items-center py-1.5 border-b border-gray-200">
											...
											<span className="font-bold text-gray-900">
												{
													COURT_POSITIONS.find((p) => p.value === courtPosition)
														?.label
												}
											</span>
										</div>
										<div className="flex justify-between items-center py-1.5 border-b border-gray-200">
											...
											<span className="font-bold text-gray-900">
												{
													MATCH_CONTEXTS.find((c) => c.value === matchContext)
														?.label
												}
											</span>
										</div>
